...
def points_interval( data_num, data_den, thresh = 0.2 ):
    ''' 
...
def decide_color( data_eff, data_fit, thresh = [0.2, 0.2],
        green = 0.5, orange = 0.3 ):
    '''
    ...
    num_points = points_interval( data_eff, data_fit, thresh_int )

    # Get the starting point of the plateau
    plat_eff = index_startplat( data_eff, thresh_plt )
    plat_fit = index_startplat( data_fit, thresh_plt )

    frac_ratio = num_points / data_fit.shape[0]
    frac_plat_eff = ( size_eff - plat_eff ) / size_eff
    frac_plat_fit = ( size_fit - plat_fit ) / size_fit

    print( frac_ratio, frac_plat_eff, frac_plat_fit )
    choose_color( frac_ratio, frac_plat_eff, green, orange )
...
def choose_color( frac_ratio, frac_plat, green, orange ):
    ...
    if frac_ratio >= green and frac_plat >= green:
        print( 'It is a green' )
    elif orange <= frac_ratio <= green and \
            orange <= frac_plat <= green:
